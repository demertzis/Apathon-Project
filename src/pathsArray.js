...
export default function pathsArray(
  ...
  let workingArray = paths.features
    .filter(
      (path) =>
        distance(currentPos, {
          lat: Number(
            path.polyline.slice(0, path.polyline.indexOf(' ')).split(',')[1]
          ),
          lng: Number(
            path.polyline.slice(0, path.polyline.indexOf(' ')).split(',')[0]
          ),
        }) < 40.0
  ...
  let crossArray = new Array(workingArray.length).fill(
    new Array(workingArray.length)
  );
  ...
  function findPath(path = null, startPos = null, offset = 0.05) {
    let polygone1 = polygon([
      concaveman(
        pathTransform(workingArray[path[path.length - 1]].polyline, offset),
        2,
        0
      ),
    ]);
